...
export const getScores = async(texts:iEmbeddedText[], config:iSpcConfig) => {
    ...
    if(predictor) return [] // Skips scoring if predictor is available.
    if(!attributes) throw new Error('Labels not provided.')

    const rawScores:any[] = []

    // Call Ollama to get scores.
    const scoreText = async(text:string, log?:boolean) => {
        const prompt = getScorePrompt({ intro, attributes, text, itemName })

        // Test output.
        try {
            const scoreOutput = await callOllama(prompt, log)
    ...
    for (const idx in texts) {
        console.log(`Scoring text ${idx} of ${texts.length}`)
        const text = texts[idx]
        const scores = await scoreText(text.text, idx === '0')
        if(!scores) continue
...
export const predictLabels = async(texts:iEmbeddedText[], scoredTexts:iScoredText[], config:iSpcConfig) => {
    ...
    const trainPredictors = async(scoredTexts:iScoredText[], labels:string[]) => {
        ...
        const y = scoredTexts.map(({ scores }) => 
            labels.map(l => {
                const score = scores.find(({ label }) => label === l)
                if (score === undefined) console.log(l)
                return score?.score!
            })
        ).filter((d) => !d.some(v => v === undefined)) // Filter out texts with missing scores.
...
const iterateVerticals = async(texts:iEmbeddedText[], config:iSpcConfig) => {
    ...
    if(!verticals.length) {
        ...
        const {labeledTexts} = await predictLabels(texts, scoredTexts, config)
        return labeledTexts
    }
    ...
    for (const vertical of verticals) {
        ...
        const {labeledTexts} = await predictLabels(texts, scoredTexts, verticalConfig)
